...
func TestNodeLifeCycle(t *testing.T) {
	stack, err := NewTestNew(testNodeConfig())
	if err != nil {
	...
	if err := stack.Start(context.Background()); err != ErrNodeRunning {
...
func TestNodeUsedDataDir(t *testing.T) {
	...
	if err := duplicate.Start(context.Background()); err != ErrDatadirUsed {
...
func TestServiceConstructionAbortion(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		if err := stack.Start(context.Background()); err != failure {
