...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [aqua/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [aqua/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [aqua/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [aqua/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [aqua/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [aqua/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [aqua/63] Channel receiving inbound node state data
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	if atomic.CompareAndSwapInt32(&d.notified, 0, 1) {
		log.Info("Block synchronization started after " + log.TimeSinceBoot().String())
	}
