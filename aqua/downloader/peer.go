...
type peerConnection struct {
	...
	version int         // Aqua protocol version number to switch strategies
	log     log.LoggerI // Contextual logger to add extra infos to peer logs
	lock    sync.RWMutex
}
...
func newPeerConnection(id string, version int, peer Peer, logger log.LoggerI) *peerConnection {
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [aqua/62+] requested on aqua/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [aqua/62+] requested on aqua/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 64 {
		panic(fmt.Sprintf("body fetch [aqua/64+] requested on aqua/%d", p.version))
	}
	// Short circuit if the peer is already fetching
