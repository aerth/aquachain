...
func (msge *msgEventer) ReadMsg() (Msg, error) {
	msg, err := msge.MsgReadWriter.ReadMsg()
	if err != nil {
		return msg, err
	}
...
// interface
func (msge *msgEventer) Close() error {
	if msge.MsgReadWriter == nil {
		return nil
	}
	if v, ok := msge.MsgReadWriter.(io.Closer); ok {
		return v.Close()
	}
	return nil
}
