...
const (
	checkpointInterval = params.OneHourBlocks // Number of blocks after which to save the vote snapshot to the database
...
var (
	...
	blockPeriod = uint64(15)    // Unused: Default minimum difference between two consecutive block's timestamps
...
type Clique struct {
	...
	signer common.Address // AquaChain address of the signing key
...
func New(config *params.CliqueConfig, db aquadb.Database) *Clique {
...
func (c *Clique) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for {
		...
		if number == c.config.StartNumber { // eg: 0
			log.Info("Welcome to AquaChain Clique", "signer", c.signer.Hex(), "blockNumber", number)
			startgenesis := chain.GetHeaderByNumber(number)
			...
			log.Info("Valid Genesis Signers:", "signers", len(signers), "addrs", signers)
			snap = newSnapshot(c.config, c.signatures, 0, startgenesis.Hash(), signers)
...
func (c *Clique) Prepare(chain consensus.ChainReader, header *types.Header) error {
	// log.Debug("Preparing Clique block", "number", header.Number, "hash", header.Hash(), "extra", len(header.Extra))
	...
	for signer := range snap.Signers {
		log.Debug("Preparing Block", "number", snap.Number, "hash", snap.Hash, "signer", signer.Hex(), "recents", len(snap.Recents))
	}
	if number%c.config.Epoch != 0 {
		c.lock.RLock()
...
// current signer.
func (c *Clique) CalcDifficulty(chain consensus.ChainReader, time uint64, parent, grandparent *types.Header) *big.Int {
...
var _ consensus.Engine = (*Clique)(nil)
