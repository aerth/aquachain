...
var chaincfg = params.AllCliqueProtocolChanges
...
// mapped from textual names used in the tests below to actual AquaChain private
// keys capable of signing transactions.
type testerAccountPool struct {
	accounts map[string]*btcec.PrivateKey
}

func newTesterAccountPool() *testerAccountPool {
	return &testerAccountPool{
		accounts: make(map[string]*btcec.PrivateKey),
	}
}
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the AquaChain address
	return crypto.PubkeyToAddress(ap.accounts[account].PubKey())
}
...
func (r *testerChainReader) GetHeaderByNumber(number uint64) *types.Header {
	if number == 0 {
		x := core.GetHeaderNoVersion(r.db, core.GetCanonicalHash(r.db, 0), 0)
		x.SetVersion(byte(r.Config().GetBlockVersion(x.Number)))
		return x
	}
	panic("not supported")
}
...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j, vote := range tt.votes {
			num := big.NewInt(int64(j) + 1)
			headers[j] = &types.Header{
				Number:   num,
				Time:     big.NewInt(int64(j) * int64(blockPeriod)),
				Coinbase: accounts.address(vote.voted),
				Extra:    make([]byte, extraVanity+extraSeal),
				Version:  chaincfg.GetBlockVersion(num),
			}
			if j > 0 {
