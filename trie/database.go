...
type Database struct {
	diskdb aquadb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb aquadb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) Dereference(child common.Hash, parent common.Hash) {
	...
	log.Trace("Dereferenced trie from memory database", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "dur", time.Since(start),
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	logger := log.Info
	if !report {
		logger = log.DebugSlow
	}

	logger("Persisted trie from mem db", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "dur", time.Since(start),
		"gcnodes", db.gcnodes, "gcsize", db.gcsize, "gctime", db.gctime, "livenodes", len(db.nodes), "livesize", db.nodesSize)
