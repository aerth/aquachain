...
type BlockChain struct {
	...
	db     aquadb.Database // Low level persistent database to store final content in
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if header == nil {
				return NonStatTy, errors.New("header nil")
			}
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	const warnAtBlockReorg = 0 // (2)
	// Ensure the user sees large reorgs
	isReorg := len(oldChain) > 0 && len(newChain) > 0
	if isReorg {
		logFn := log.Debug
		if len(oldChain) > warnAtBlockReorg || len(deletedTxs) > 0 {
			logFn = log.Warn
		}
	...
	for i := len(newChain) - 1; i >= 0; i-- {
		...
		if e := WriteTxLookupEntries(bc.db, newChain[i]); e != nil {
			err = fmt.Errorf("reorg: failed to write lookup metadata: %v", e)
			break
		}
		addedTxs = append(addedTxs, newChain[i].Transactions()...)
	}

	// regardless of WriteTxLookupEntries error
	diff := types.TxDifference(deletedTxs, addedTxs)

	// WriteTxLookupEntries error
	if err != nil {
		// send alert notification
		go doAlert(oldChain, newChain, deletedTxs, deletedLogs, commonBlock, nil, err)
		alerts.Warnf("Failed to insert reorged chain: %v", err)
		return err
	}
	...
	for _, tx := range diff {
		...
		diffs = append(diffs, tx.Hash().String())
	}
	go doAlert(oldChain, newChain, deletedTxs, deletedLogs, commonBlock, diffs, err)
	if len(diffs) > 0 {
		alerts.Warnf("Deleted tx hashes:\n%q\n", diffs)
	}
...
func doAlert(oldChain, newChain types.Blocks, deletedTxs types.Transactions,
	deletedLogs []*types.Log, commonBlock *types.Block, diffs []string, err error) {
	value := big.NewInt(0)
	for _, newblock := range newChain {
		for _, newtx := range newblock.Body().Transactions {
			value.Add(value, newtx.Value())
		}
	}
	alerts.Warnf("⚠️WARNING: reorg detected (%s %s) after height %d: %d blocks rolled back (%d deleted logs, %d deleted txs), added %d new blocks",
		common.WeiToCoin(value), "AQUA",
		commonBlock.NumberU64(), len(oldChain), len(deletedLogs), len(deletedTxs),
		len(newChain))
	if len(deletedTxs) == 0 {
		return
	}
	...
	alerts.Warnf("Deleted tx hashes (appears to be %s value):\n%q\n", common.WeiToCoin(value), diffs)
}
